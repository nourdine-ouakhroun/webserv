...
void ServerRun::ParssingRecuistContent(std::string ContentRequist)
{
	...
	// }
	size_t position = _data.at(0).getValue().find(':');
	// std::cout << "-------------- " <<  << " ----------" << std::endl;
	std::vector<ServerModel> smodel = serves.getServersByPort((unsigned short)strtol(_data[0].getValue().substr(position + 1).c_str(), NULL, 10));
